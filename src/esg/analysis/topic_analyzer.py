...
from src.esg.config import ESG_DIMENSION_NAMES, ESG_DIMENSIONS, MOCK_DATA_DIR
...
class TopicAnalyzer:
    ...
    def analyze_trends(self, category: Optional[str] = None) -> Dict[str, Any]:
        ...
        result = {
            ...
            "hot_topics": [
                {"id": t.id, "name": t.name, "heat_score": t.heat_score, "trend": t.trend_direction}
            ...
            "avg_growth": round(sum(t.growth for t in filtered) / len(filtered) * 100, 1),
        }
    ...
    def generate_wordcloud_data(
        self, category: Optional[str] = None, min_weight: float = 0.0
    ) -> List[WordCloudItem]:
        """生成词云数据
        ...
        return [
            WordCloudItem(
                text=t.name, value=self._calculate_word_size(t.heat_score), category=t.category
            )
