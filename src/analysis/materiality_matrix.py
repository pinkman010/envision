...
from src.config import DEFAULT_MATERIALITY_SCORES, ESG_COLORS
...
class MaterialityMatrix:
    ...
    def get_priority_list(self) -> List[Dict[str, Any]]:
        ...
        topics.sort(key=lambda x: (priority_order.get(x["priority"], 3), -x["financial_score"], -x["impact_score"]))
        return topics
