...
3. **差距诊断**: 对标行业标杆识别差距
   - 查看业务单元风险映射详情
   - 查看国际标准合规检查清单
4. **AI策略**: 生成针对性改进策略（含多语言报告生成）
5. **沟通时机**: 基于策略主题推荐最佳披露时机
6. **RAG问答**: 基于知识库回答ESG问题
...
> 📌 **后续版本计划**: 
> - 添加更多语言支持（日语、韩语、德语、法语、西班牙语）

### ⚠️ 待集成功能（已实现但未接入UI）
以下功能已在代码中实现，但尚未接入 Streamlit UI：

#### 分析模块 (src/esg/analysis/)
- **自动更新器** (`auto_updater.py`): 自动检测并更新ESG议题数据
- **通道顾问** (`channel_advisor.py`): ESG沟通渠道建议
- **竞争分析器** (`competitor_analyzer.py`): 竞争对手分析
- **物质性矩阵** (`materiality_matrix.py`): 重要性议题矩阵分析
- **主题更新器** (`topic_updater.py`): 议题数据动态更新

#### 数据提取模块 (src/esg/extraction/)
- **碳足迹计算** (`carbon_footprint.py`): 支持范围1/2/3排放计算、碳强度分析、行业基准比较
- **异步PDF提取** (`pdf_extractor_async.py`): 批量并发处理PDF

#### 融合模块 (src/esg/fusion/)
- **规则引擎** (`rule_engine.py`): 基于规则的数据融合与推理

#### 补全模块 (src/esg/completion/)
- **数据补全** (`data_completion.py`): 智能补全缺失的ESG数据
