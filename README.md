...
| 阶段 | 功能 | 状态 |
|------|------|------|
| P0 / 中期 Demo | 单报告分析 `single_report_analysis` | 已实现 |
| P1 / 中期后 | 对话窗口（检索问答） | 待实现 |
| P2 / 结项阶段 | 行业竞品对标与分析 | 预留接口，未完成 |
| P2 / 结项阶段 | 披露标准版本差异分析 | 待实现 |
...
- 用远景 ESG 报告跑通 `single_report_analysis` 端到端
- 修复报错并打磨 Streamlit 展示
- 在 P0 稳定后再推进对话窗口（P1）
- 结项前整理 3 到 4 篇友商报告导入 `peer_reports`
