...
from src.esg.extraction.pdf_extractor import PDFContent, PDFExtractionError, PDFExtractor
from src.esg.utils.performance_monitor import PDF_EXTRACTION_DURATION, get_monitor
...
class AsyncPDFExtractor:
    ...
    def __init__(self, max_workers: int = 4, preferred_backend: Optional[str] = None):
        """初始化异步PDF提取器
    ...
    async def extract_async(self, pdf_path: Union[str, Path]) -> PDFContent:
        ...
        try:
            ...
            # 记录指标
            PDF_EXTRACTION_DURATION.labels(backend=self.preferred_backend or "auto").observe(
                content.metadata.total_pages * 0.1
            )  # 估算时间
    ...
    async def extract_directory_async(
        ...
        return await self.extract_batch_async(pdf_files, progress_callback=progress_callback)
...
class PDFExtractionPipeline:
    ...
    async def process(self, pdf_path: Union[str, Path]) -> PDFContent:
        ...
        for preprocessor in self.preprocessors:
            processed_path = await self._run_sync_or_async(preprocessor, processed_path)
    ...
    async def process_batch(
        self, pdf_paths: List[Union[str, Path]], progress_callback: Optional[callable] = None
    ) -> Dict[str, Union[PDFContent, Exception]]:
        """批量处理PDF文件
    ...
    async def _run_sync_or_async(self, func: callable, *args, **kwargs) -> Any:
        """运行同步或异步函数"""
