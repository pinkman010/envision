...
class RuleContext:
    ...
    def add_history(self, rule_name: str, status: RuleStatus, details: Dict[str, Any] = None) -> None:
        """添加执行历史"""
        self.history.append({
            "rule": rule_name,
            "status": status.name,
        ...
        try:
            ...
            if tag_filter:
                execution_order = [
                    ...
                    if tag_filter in self._rules[name].tags
                ]
...
class ESGRules:
    ...
    def carbon_intensity_rule() -> Rule:
        """碳排放强度计算规则"""
        return RuleBuilder().name("carbon_intensity")\
            .description("计算单位营收碳排放强度")\
            .with_tags("environment", "calculation")\
            .when(lambda ctx: ctx.metrics is not None 
                  and ctx.metrics.carbon_emissions is not None)\
            .then(lambda ctx: ctx.set_result(
                "carbon_intensity",
                {
                    "value": ctx.metrics.carbon_emissions / 1000000,  # 转换为吨/百万元
                    "unit": "吨CO2e/百万元",
                    "assessment": "高" if ctx.metrics.carbon_emissions > 5000000 else "中" if ctx.metrics.carbon_emissions > 1000000 else "低"
                }
            ))\
            .build()
    ...
    def diversity_score_rule() -> Rule:
        """多元化得分计算规则"""
        return RuleBuilder().name("diversity_score")\
            .description("计算员工多元化得分")\
            .with_tags("social", "calculation")\
            .when(lambda ctx: ctx.metrics is not None 
                  and ctx.metrics.female_ratio is not None)\
            .then(lambda ctx: ctx.set_result(
                "diversity_score",
                {
                    "score": min(ctx.metrics.female_ratio * 100, 100),
                    "level": "优秀" if ctx.metrics.female_ratio >= 0.4 else "良好" if ctx.metrics.female_ratio >= 0.3 else "待改进"
                }
            ))\
            .build()
    ...
    def governance_compliance_rule() -> Rule:
        """治理合规检查规则"""
        return RuleBuilder().name("governance_compliance")\
    ...
    def _check_governance(context: RuleContext) -> None:
        ...
        context.set_result("governance_compliance", {
    ...
    def data_quality_check_rule() -> Rule:
        """数据质量检查规则"""
        return RuleBuilder().name("data_quality_check")\
    ...
    def _check_data_quality(context: RuleContext) -> None:
        ...
        completeness = 1 - len(missing_fields) / (len(env_fields) + len(social_fields) + len(gov_fields))
        
        context.set_result("data_quality", {
            "completeness": round(completeness, 2),
            "missing_fields": missing_fields,
            "quality_level": "高" if completeness >= 0.8 else "中" if completeness >= 0.5 else "低"
        })
