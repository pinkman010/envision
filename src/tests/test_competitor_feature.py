...
def test_competitor_analyzer():
    logger.info('=== 测试竞争对手深度分析功能 ===')

    # 1. 测试 CompetitorAnalyzer
    analyzer = CompetitorAnalyzer()
    logger.info('1. CompetitorAnalyzer 初始化成功')
    logger.info(f'   可用竞争对手: {analyzer.get_competitor_list()}')
    ...
    logger.info('3. 文字分析生成成功!')
    logger.info(f'   分析内容长度: {len(analysis)} 字符')
    logger.info('   分析内容预览:')
    for i, para in enumerate(analysis.split('\n\n')[:3], 1):
        logger.info(f'   段落{i}: {para[:100]}...')

    # 4. 测试对比表格
    table = analyzer.generate_comparison_table(metrics, '维斯塔斯', gap_data)
    logger.info('4. 对比表格生成成功!')
    logger.info(f'   表格行数: {len(table)}')
    for row in table:
        logger.info(f"   - {row['维度']}: {row['我司现状']} vs 标杆, 差距{row['差距']}, {row['改进机会']}")

    # 5. 测试创新亮点
    highlights = analyzer.get_innovation_highlights('维斯塔斯')
    logger.info('5. 创新亮点获取成功!')
    for i, h in enumerate(highlights[:3], 1):
        logger.info(f"   {i}. {h}")

    # 6. 测试整体对比
    comparison = analyzer.get_overall_comparison(metrics)
    logger.info('6. 整体对比数据生成成功!')
    logger.info(f"   当前公司: {comparison['current_company']['name']}")
    logger.info(f"   综合评分: {comparison['current_company']['overall_score']}")
    logger.info(f"   排名: #{comparison['current_company']['rank']} / {comparison['current_company']['total_companies']}")
