...
def test_compliance_checker():
    ...
    for i, (std_id, result) in enumerate(list(results.items())[:5]):
        logger.info(
            f"  {std_id}: {result['status']} (得分: {result['score']}, 类型: {result['requirement_type']})"
        )
    ...
    logger.info(
        f"已合规: {summary['compliant_count']}, 部分合规: {summary['partial_count']}, 未合规: {summary['non_compliant_count']}"
    )
