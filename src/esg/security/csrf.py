...
class CSRFProtection:
    ...
    def generate_token(self, session_id: str, timestamp: Optional[int] = None) -> str:
        ...
        # 创建HMAC签名
        signature = hmac.new(self.secret_key, message.encode(), hashlib.sha256).hexdigest()[:16]
    ...
    def validate_token(self, token: str, session_id: str, max_age: Optional[int] = None) -> bool:
        """验证CSRF令牌
...
class DoubleSubmitCookie:
    ...
    def get_js_config(self, cookie_value: str) -> dict:
        ...
        return {"cookie_name": self.COOKIE_NAME, "header_name": "X-CSRF-Token", "token": parts[1]}


# 便捷函数


def generate_csrf_token(session_id: str, secret_key: Optional[str] = None) -> str:
    """生成CSRF令牌"""
...
def validate_csrf_token(
    token: str, session_id: str, secret_key: Optional[str] = None, max_age: Optional[int] = None
) -> bool:
    """验证CSRF令牌"""
    protection = CSRFProtection(secret_key)
