...
class ComplianceChecker:
    ...
    def comprehensive_check(self, disclosure_data: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """综合合规性检查
        ...
        for std_type in StandardType:
            if std_type.value in disclosure_data:
                reports[std_type.value] = self.standards_manager.generate_compliance_report(
                    std_type, disclosure_data[std_type.value]
                )

        # 计算整体合规率
        total_requirements = sum(r["summary"]["total_requirements"] for r in reports.values())
        total_compliant = sum(r["summary"]["compliant"] for r in reports.values())
