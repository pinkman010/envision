...
class CompetitorAnalyzer:
    ...
    def get_strategy_by_dimension(
        self, company_name: str, dimension: str
    ...
    def generate_analysis(
        self, current_metrics: ESGMetrics, benchmark_company: str, gap_data: Dict[str, Any]
    ) -> str:
        """生成深度对标分析报告
            ...
            if gap_value > self.GAP_THRESHOLD:
                ...
                if strategy:
                    ...
                    analysis = self.ANALYSIS_TEMPLATE.format(
                        ...
                        最佳实践描述=(
                            strategy.best_practice_description[:100] + "..."
                            if len(strategy.best_practice_description) > 100
                            else strategy.best_practice_description
                        ),
                        关键成果=(
                            strategy.key_results.split("。")[0]
                            if "。" in strategy.key_results
                            else strategy.key_results.split("，")[0]
                        ),
        ...
        if not analysis_parts:
            return (
                "贵司各维度表现与标杆企业差距较小，建议持续优化现有举措。参考行业最佳实践："
    ...
    def _generate_summary(
        self, analysis_parts: List[Dict[str, Any]], benchmark_company: str
    ) -> str:
        """生成总结建议
    ...
    def generate_comparison_table(
        ...
        for dim in dimensions:
            ...
            strategy = self.get_strategy_by_dimension(benchmark_company, dim)
            benchmark_practice = (
                strategy.best_practice_description[:80] + "..." if strategy else "暂无数据"
            )
    ...
    def get_overall_comparison(self, current_metrics: ESGMetrics) -> Dict[str, Any]:
        """获取整体对比数据
        ...
        Returns:
            整体对比数据
        """
        current_overall = sum(current_metrics.get_dimension_score(d) for d in ["E", "S", "G"]) / 3
        ...
        # 计算排名
        all_scores = [
            (comparison["current_company"]["name"], comparison["current_company"]["overall_score"])
        ]
        ...
        rank = next(
            i for i, (name, _) in enumerate(all_scores, 1) if name == current_metrics.company_name
        )
