...
from src.ui.state import init_session_state, get_state_manager
...
from src.extractor.pdf_extractor import PDFExtractor, PDFContent
...
from src.analysis.strategy_generator import StrategyGenerator, StrategyPriority
...
def render_manual_input() -> None:
    ...
    with st.form("manual_input_form"):
        ...
        with cols[0]:
            ...
            renewable = st.number_input("可再生能源占比(%)", min_value=0.0, max_value=100.0, value=40.0)
        with cols[1]:
        ...
        with cols[1]:
            ethics = st.number_input("道德培训覆盖率(%)", min_value=0.0, max_value=100.0, value=70.0)
        with cols[2]:
            report_quality = st.number_input("ESG报告质量评分", min_value=0.0, max_value=100.0, value=75.0)
...
def render_metrics_overview() -> None:
    ...
    for i, dim in enumerate(["E", "S", "G"]):
        with cols[i + 1]:
            render_score_card(ScoreCardData(
    ...
    with st.expander("查看详细指标"):
        ...
        with cols[0]:
            st.markdown("**🌱 环境指标**")
            st.write(f"碳排放: {metrics.carbon_emissions:,.0f} 吨" if metrics.carbon_emissions else "碳排放: N/A")
            st.write(f"可再生能源: {metrics.renewable_energy_ratio:.1%}" if metrics.renewable_energy_ratio else "可再生能源: N/A")
            st.write(f"能源效率: {metrics.energy_efficiency:.1f}%" if metrics.energy_efficiency else "能源效率: N/A")
        
        with cols[1]:
            st.markdown("**👥 社会指标**")
            st.write(f"员工数: {metrics.employee_count:,} 人" if metrics.employee_count else "员工数: N/A")
            st.write(f"女性比例: {metrics.female_ratio:.1%}" if metrics.female_ratio else "女性比例: N/A")
            st.write(f"培训时长: {metrics.training_hours:.1f} 小时" if metrics.training_hours else "培训时长: N/A")
        
        with cols[2]:
            st.markdown("**⚖️ 治理指标**")
            st.write(f"独董比例: {metrics.board_independence_ratio:.1%}" if metrics.board_independence_ratio else "独董比例: N/A")
            st.write(f"伦理培训: {metrics.ethics_training_coverage:.1%}" if metrics.ethics_training_coverage else "伦理培训: N/A")
...
def render_weights_page(config: Dict[str, Any]) -> None:
    ...
    render_header(
        title="权重配置",
        subtitle="使用AHP层次分析法科学配置ESG维度权重"
    )
...
def render_simple_weights_config() -> None:
    ...
    st.write(f"**归一化后**: E={normalized['E']:.0%}, S={normalized['S']:.0%}, G={normalized['G']:.0%}")
...
def render_gap_page(config: Dict[str, Any]) -> None:
    ...
    with st.expander("🔍 查看业务单元风险映射详情"):
        try:
                    ...
                    with st.container():
                        ...
                        with cols[1]:
                            ...
                            for risk in top_risks:
                                color = risk["color"]
                                risk_texts.append(f"<span style='color:{color};font-weight:bold;'>{risk['topic_name']}</span> ({risk['impact_level']})")
            ...
            matrix_df_data = []
            for row in matrix_data:
                row_data = {"业务单元": row["business_unit"][:8] + "..." if len(row["business_unit"]) > 8 else row["business_unit"]}
                ...
                if not non_compliant:
                    ...
                    for item in non_compliant[:10]:
                        with st.container():
                            ...
                            with cols[0]:
                                req_type_color = "red" if item['requirement_type'] == '强制' else 'orange'
                                st.markdown(f"**{item['clause_name'][:20]}...**")
                                st.markdown(f"<span style='color:{req_type_color};font-size:0.8em;'>[{item['requirement_type']}]</span>", unsafe_allow_html=True)
                            with cols[1]:
                                status_color = {"未合规": "red", "部分合规": "orange"}.get(item['status'], "gray")
                                st.markdown(f"<span style='color:{status_color};font-weight:bold;'>{item['status']}</span>", unsafe_allow_html=True)
                            with cols[2]:
...
def render_strategies_page(config: Dict[str, Any]) -> None:
    ...
    with cols[1]:
        render_metric_card("高优先级", priority_counts["高"], "个", delta=1, delta_description="需重点关注")
    with cols[2]:
    ...
    with cols[4]:
        est_impact = sum(s.expected_impact for s in strategies) / len(strategies) if strategies else 0
    ...
    for i, strategy in enumerate(sorted_strategies, 1):
        with st.container():
            render_strategy_card(StrategyCardData(
                id=strategy.id,
...
def create_metrics_from_extraction(result: Any) -> ESGMetrics:
    ...
    return ESGMetrics(
        ...
        safety_incidents=int(get_value("safety_incidents", 0)) if get_value("safety_incidents") else None,
