...
class AutoUpdater:
    ...
    def _get_update_files(self) -> List[Path]:
        ...
        if self._update_files is None:
            if self.updates_dir.exists():
                self._update_files = sorted([f for f in self.updates_dir.glob("update_*.json")])
            else:
                self._update_files = []
    ...
    def _generate_mock_update(
        ...
        for topic in random.sample(topics, min(changes_count, len(topics))):
            ...
            heat_change = (
                round(random.uniform(-3, 8), 1)
                if trend == "up"
                else (
                    round(random.uniform(-5, 2), 1)
                    if trend == "down"
                    else round(random.uniform(-1, 1), 1)
                )
            )
