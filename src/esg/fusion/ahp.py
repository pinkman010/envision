...
from src.esg.config import AHP_RI_TABLE, AHP_CONSISTENCY_THRESHOLD
...
class AHPFusionEngine:
    ...
    def build_matrix(
        ...
        if validate:
            ...
            for (i, j), val in comparisons.items():
                if not (1/9 <= val <= 9):
                    raise ValueError(
                        f"比较值必须在[1/9, 9]范围内，got ({i},{j}): {val}"
                    )
                if i >= j:
                    raise ValueError(
                        f"只接受上三角矩阵比较，要求 i < j，got ({i}, {j})"
                    )
                if i >= len(labels) or j >= len(labels):
                    raise ValueError(
                        f"索引超出范围，最大索引为 {len(labels)-1}"
                    )
        ...
        for i in range(matrix.shape[0]):
            for j in range(i + 1, matrix.shape[1]):
                if not np.isclose(matrix[i, j] * matrix[j, i], 1.0, rtol=1e-5):
                    raise ValueError(
                        f"矩阵不满足互反性: matrix[{i},{j}] * matrix[{j},{i}] != 1"
                    )
    ...
    def auto_correct_matrix(
        ...
        raise ValueError(
            f"经过{max_iterations}次迭代仍无法达到一致性要求，"
            "请手动调整判断矩阵"
        )
    ...
    def sensitivity_analysis(
        ...
        for i, label in enumerate(self.labels):
            stats[label] = {
                ...
                "cv": float(np.std(weights_array[:, i]) / np.mean(weights_array[:, i])) if np.mean(weights_array[:, i]) > 0 else 0
            }
