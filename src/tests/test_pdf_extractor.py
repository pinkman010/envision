...
class TestPDFExtractor(unittest.TestCase):
    ...
    @patch("src.esg.extraction.pdf_extractor.HAS_PDFPLUMBER", False)
    @patch("src.esg.extraction.pdf_extractor.HAS_PYPDF2", False)
    ...
    @patch("src.esg.extraction.pdf_extractor.HAS_PDFPLUMBER", True)
    @patch("src.esg.extraction.pdf_extractor.pdfplumber")
    ...
    @patch("src.esg.extraction.pdf_extractor.HAS_PYPDF2", True)
    ...
    def test_extract_with_pypdf2(self):
        ...
        # 临时替换模块中的PyPDF2
        import src.esg.extraction.pdf_extractor as pdf_module
    ...
    def test_extract_batch(self):
        ...
        with patch.object(PDFExtractor, "extract") as mock_extract:
            mock_extract.return_value = PDFContent(
                text="测试内容", metadata=PDFMetadata(filename="test.pdf")
            )
    ...
    def test_extract_batch_with_failure(self):
        ...
        with patch.object(PDFExtractor, "extract") as mock_extract:
            ...
            def side_effect(path):
                ...
                return PDFContent(text="测试内容", metadata=PDFMetadata(filename="test.pdf"))
    ...
    def test_pdf_content_dataclass(self):
        """测试PDFContent数据类"""
        content = PDFContent(
            text="测试文本", metadata=PDFMetadata(filename="test.pdf"), pages=["页1", "页2"]
        )
