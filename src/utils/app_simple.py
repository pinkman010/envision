...
from src.config import ESG_DIMENSION_NAMES, BENCHMARK_COMPANIES, ANALYSIS_YEARS
...
def render_analysis_step(
    ...
    try:
        ...
        with st.spinner("💡 正在生成改进策略..."):
            ...
            # 转换为字典列表
            strategy_list = [
                strategy_gen.to_dict(s) for s in strategies
            ]
        ...
        overall_score = sum(
            scores[d] * config["weights"][d] for d in ["E", "S", "G"]
        )
...
def load_demo_data(case_type: str) -> PDFContent:
    ...
    from src.extractor.pdf_extractor import PDFMetadata
...
def render_simple_app() -> None:
    ...
    render_header(
        title="ESG分析报告系统",
        subtitle="上传ESG报告，智能分析企业可持续发展表现"
    )
