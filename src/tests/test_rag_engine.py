...
class TestRAGEngine(unittest.TestCase):
    ...
    def test_init_default(self):
        """测试默认初始化"""
        with patch("src.esg.rag.engine.ChromaDBStore") as mock_store:
    ...
    def test_init_with_custom_model(self):
        """测试自定义模型初始化"""
        with patch("src.esg.rag.engine.ChromaDBStore"):
    ...
    @patch("src.esg.rag.engine.requests.post")
...
这是最终答案"""
    ...
    def test_extract_thinking_no_format(self, mock_post):
        """测试无格式输出处理"""
        mock_response = MagicMock()
        mock_response.json.return_value = {"response": "这是一个没有特定格式的简单回答"}
        mock_response.raise_for_status = MagicMock()
        mock_post.return_value = mock_response
    ...
    def test_calculate_confidence(self):
        ...
        # 测试高置信度文档
        high_conf_docs = [{"score": 0.9}, {"score": 0.8}, {"score": 0.85}]
        confidence = engine._calculate_confidence(high_conf_docs)
        self.assertGreater(confidence, 0.5)
        self.assertLessEqual(confidence, 1.0)

        # 测试低置信度文档
        low_conf_docs = [{"score": 0.5}, {"score": 0.6}]
    ...
    def test_rag_response_dataclass(self):
        ...
        response = RAGResponse(
            answer="测试答案", reasoning="测试推理", sources=[{"text": "来源1"}], confidence=0.85
        )
...
class TestRAGEngineEdgeCases(unittest.TestCase):
    ...
    def test_query_special_characters(self, mock_post):
        ...
        mock_store = MockChromaDBStore([{"text": "文档", "metadata": {}, "score": 0.8}])
        engine = RAGEngine(model="test-model", store=mock_store)
    ...
    def test_query_unicode_content(self, mock_post):
        ...
        mock_store = MockChromaDBStore([{"text": "🌍 环境内容", "metadata": {}, "score": 0.8}])
        engine = RAGEngine(model="test-model", store=mock_store)
