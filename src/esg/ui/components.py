...
from src.esg.config import ESG_DIMENSION_NAMES, ESG_COLORS
...
def render_strategy_card(
    ...
    with st.container():
        ...
        with col1:
            ...
            st.markdown(f"<div style='{title_style}'>**{priority_emoji} {data.title}**</div>", unsafe_allow_html=True)
        ...
        if data.communication_style:
            style_colors = {"正式": "#595959", "亲和": "#52c41a", "技术": "#1890ff", "营销": "#fa8c16"}
        ...
        with st.expander("查看详情", expanded=expanded):
            ...
            st.markdown(f"<div style='{content_style}'>**描述**: {data.description}</div>", unsafe_allow_html=True)
            
            if data.timeframe:
                st.markdown(f"<div style='{content_style}'>**预计周期**: {data.timeframe}</div>", unsafe_allow_html=True)
            
            st.markdown("**行动项**:")
            for action in data.actions:
                st.markdown(f"<div style='{content_style}'>- {action}</div>", unsafe_allow_html=True)
            ...
            if data.recommended_channels:
                ...
                # 分离主渠道和辅助渠道
                primary_channels = [c for c in data.recommended_channels if c.get("priority") == "主渠道"]
                supporting_channels = [c for c in data.recommended_channels if c.get("priority") == "辅助渠道"]
                ...
                # 显示主渠道（2-3个）
                if primary_channels:
                    st.markdown("<div style='font-size: 0.85em; color: #666; margin-bottom: 4px;'>主渠道</div>", unsafe_allow_html=True)
