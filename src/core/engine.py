...
class ESGAnalysisEngine:
    ...
    def _validate_data_quality(self, metrics: ESGMetrics) -> None:
        ...
        if metrics.carbon_emissions is not None:
            if metrics.carbon_emissions < 1000:
                ...
                self.warnings.append(
                    f"碳排放数值({metrics.carbon_emissions})异常巨大，请核实单位"
                )
        ...
        if missing_dimensions:
            self.warnings.append(
                f"以下维度缺少数据: {', '.join(missing_dimensions)}"
            )
    ...
    def _analyze_gaps(
        ...
        overall_current: float = sum(
            gaps["dimensions"][dim]["current"] * self.weights[dim]
            for dim in ["E", "S", "G"]
        )
        overall_target: float = sum(
            gaps["dimensions"][dim]["target"] * self.weights[dim]
            for dim in ["E", "S", "G"]
        )
