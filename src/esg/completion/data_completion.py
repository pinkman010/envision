...
from src.esg.config import DEFAULT_SCORE
from src.esg.core.models import BenchmarkData, ESGMetrics
...
class DataCompletionEngine:
    ...
    def complete(
        ...
        overall_confidence = (
            sum(log.confidence for log in self._logs) / len(self._logs) if self._logs else 1.0
        )
    ...
    def _try_complete_field(
        self, field_name: str, metrics: ESGMetrics
    ...
    def _complete_from_benchmark(self, field_name: str) -> Tuple[Optional[Any], float, str]:
        """从行业基准数据补全
    ...
    def _complete_from_historical(self, field_name: str) -> Tuple[Optional[Any], float, str]:
        """从历史数据趋势补全
